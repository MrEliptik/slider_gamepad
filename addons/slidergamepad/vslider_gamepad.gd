...
func handle_joystick_discrete(event: InputEvent) -> void:
	...
	if event.is_action_pressed("ui_down", true) and joystick_can_input:
		...
		if not is_zero_approx(joystick_smooth_discrete_threshold):
			...
			smooth_discrete_timer.connect("timeout", self, "on_threshold_timeout", [-1])
		get_viewport().set_input_as_handled()
...
#		if smooth_discrete_timer and smooth_discrete_timer.timeout.is_connected(on_threshold_timeout):
		if smooth_discrete_timer and smooth_discrete_timer.is_connected("timeout", self, "on_threshold_timeout"):
			smooth_discrete_timer.disconnect("timeout", self, "on_threshold_timeout")
		get_viewport().set_input_as_handled()
		
	if event.is_action_pressed("ui_up", true) and joystick_can_input:
		joystick_can_input = false
		value += 1
		if not is_zero_approx(joystick_smooth_discrete_threshold):
			smooth_discrete_timer = get_tree().create_timer(joystick_smooth_discrete_threshold)
			smooth_discrete_timer.connect("timeout", self, "on_threshold_timeout", [1])
		get_viewport().set_input_as_handled()
	elif event.is_action_released("ui_up", true):
		joystick_can_input = true
		sliding_dir = 0.0
		if smooth_discrete_timer and smooth_discrete_timer.is_connected("timeout", self, "on_threshold_timeout"):
			smooth_discrete_timer.disconnect("timeout", self, "on_threshold_timeout")
...
func handle_dpad(event: InputEvent) -> void:
	...
	if event is InputEventJoypadButton and event.button_index == JOY_DPAD_DOWN and event.is_pressed():
		value -= dpad_step
		get_viewport().set_input_as_handled()
	elif event is InputEventJoypadButton and event.button_index == JOY_DPAD_UP and event.is_pressed():
		value += dpad_step
		get_viewport().set_input_as_handled()
...
func on_threshold_timeout(direction: int) -> void:
	if direction == 1 and Input.is_action_pressed("ui_up", true) or \
		direction == -1 and Input.is_action_pressed("ui_down", true):
